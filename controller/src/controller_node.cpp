...
double actualPose[3] = {0.0, 0.0, 0.0};
double DT = 0.01;

double external_speed;
double v=0, ro=0, rc = 0;
double dist_location=1.6, dist_wheels=1.6;    
double max_steer_angle= 0.7853981634;  
double A[4],B[2],Q[4],R,Kr[2];

double buffer_vel[2][N_RET_VEL_MAX];
double evol_vel[2][N_RET_VEL_MAX]; 
...
void trajectoryCallback(const boost::shared_ptr<nav_msgs::Path const>& msg) 
{
	...
	if (init_odom==0) return;
        ...
        //Yörünge aynıysa yeniden hesaplamamak için çıkış yapılıyor.
	...
	//Aracın gerçek konumu, yörüngenin başladığı noktadır.
	...
	for (int i=1; i<m; i++)  
	 ...
	 spline_path.header.frame_id="base_footprint";
	 points_spline.header.frame_id = "base_footprint";
...
//********************************Hız İçin Oluşturulan fonksiyon***********************//
...
void odoCallBack(const sensor_msgs::JointState::ConstPtr& data){
	
	double delta_dk = cos(data->position[1]) * data->velocity[0] * DT;
	double delta_theta_k = sin(data->position[1]) * (data->velocity[0] / dist_wheels) * DT;

	actualPose[0] += (delta_dk * cos(actualPose[2] + delta_theta_k/2));
  actualPose[1] += (delta_dk * sin(actualPose[2] + delta_theta_k/2));
	actualPose[2] += delta_theta_k;
	...
	geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(actualPose[2]);
	/*geometry_msgs::TransformStamped odom_trans;
	odom_trans.header.frame_id = "base_footprint";
	odom_trans.transform.translation.x = actualPose[0];
	odom_trans.transform.translation.y = actualPose[1];
	...
	odom.pose.pose.position.x = actualPose[0];
	odom.pose.pose.position.y = actualPose[1];
...
void timerCallback(const ros::TimerEvent& e) 
{
	...
	reference_pose.header.frame_id = "base_footprint";
	...
	pose_future.header.frame_id = "base_footprint";
...
int main(int argc, char **argv) 
{
  ...
  ROS_INFO("\trc_max: %lf",rc_max);
	...
	//Odometry Subscriber
	ros::Subscriber odometry_sub = n.subscribe("/tricycle_mobile_robot/joint_states", 1000, odoCallBack);
	odometry_pub = n.advertise<nav_msgs::Odometry>("/absolute_pose", 1000, true);
	
	spline_pub = n.advertise<nav_msgs::Path> ("/spline",1000, true);
	spline_points_pub = n.advertise<visualization_msgs::Marker> ("/points_spline",1000,true);
	reference_pose_pub = n.advertise<geometry_msgs::PoseStamped> ("/reference_pose",1000,true);
	predicted_pose_pub = n.advertise<geometry_msgs::PoseStamped> ("/predicted_pose",1000,true);
	cmd_vel_pub = n.advertise<geometry_msgs::Twist> ("/cmd_vel",1,true);
	speed_pub = n.advertise<std_msgs::Float64> ("/tricycle_mobile_robot/speed_joint_controller/command",1000,true);
	steer_pub = n.advertise<std_msgs::Float64> ("/tricycle_mobile_robot/steer_joint_controller/command",1000,true);
	oe_pub = n.advertise<std_msgs::Float64> ("/oe",1000,true);
	de_pub = n.advertise<std_msgs::Float64> ("/de",1000,true);
...
//********************Bir matrisi başka bir matrise ekleme*********************************//
...
//*****************İkinci mertebeden bir matrisin tersi**********************//
...
//******************İkinci dereceden matrisin transpozu*************************//
...
//**********************ro=-K*x'i bulmak için optimal kontrol yasası uygulama**********************//
